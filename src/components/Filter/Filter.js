...
class FilterItem extends Component {
  render() {
    return (
      <ListItem innerDivStyle={this.props.current == this.props.item.name ? activeListItemStyle : listItemStyle}>
  ...
  renderFilter() {
    ...
    if (data.length > 0) {
      let filter = data.map(item => {
        return <FilterItem current={this.props.current} key={item.name} item={item} />;
      });
      return filter;
    } else {
      return (
